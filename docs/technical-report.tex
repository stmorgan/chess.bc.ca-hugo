...
% NOTE: increment \reportrevision and add a row to the revision-history
% table below with EVERY change to this report.
\newcommand{\reportrevision}{2}

\title{Technical Report on the Migration of \texttt{chess.bc.ca} to a Git
Content Management System}
\author{Scott Morgan \\ British Columbia Chess Federation}
\date{Revision \reportrevision{} --- July 11, 2026}
...
\midrule
1 & July 8, 2026 & Initial report. \\
2 & July 11, 2026 & Bulletin auto-announcement workflow; bulletin \\
  &               & publishing procedure; revision numbering. \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{GitHub Actions --- automation in the repository}

GitHub Actions runs small automated jobs directly in the repository. Two
are defined, both in \texttt{.github/workflows/}:

\begin{description}
  \item[Nightly rebuild] (\texttt{nightly-rebuild.yml}). News expiry and
  banner shuffling are decisions made \emph{at build time}. If nothing were
  ever rebuilt, an expired news item would sit on the home page forever,
  because time passing does not rebuild the site by itself. This job
  therefore fires every night at 2:17\,a.m.\ Pacific time and pings a
  Netlify \emph{build hook} (a secret URL that triggers a build). The hook
  URL is stored as a GitHub Actions secret named
  \texttt{NETLIFY\_BUILD\_HOOK}. The nightly build is what actually moves
  news into the archive on its expiry date and reshuffles the banners each
  day.

  \item[Bulletin announcements] (\texttt{announce-bulletin.yml}). When an
  editor publishes a new bulletin, this job automatically posts the
  matching news item --- ``Bulletin \#N Posted'', linking to the uploaded
  PDF --- in the same format the site has always used, with the standard
  two-week expiry. It reacts only to \emph{newly added} bulletin entries
  (editing an existing bulletin never re-announces it) and skips any
  bulletin that already has an announcement, so duplicates are impossible.
  Publishing a bulletin in the CMS is therefore the only manual step; the
  home-page announcement writes itself a minute or two later.
\end{description}
...
\textbf{An editor publishes a bulletin.} The editor creates a \emph{New
Bulletin} entry, uploading the PDF \emph{from within the entry's PDF field}
(see Section~\ref{sec:ops}), and clicks \emph{Publish}. The commit lands on
GitHub and Netlify deploys the updated bulletins index. The push also
triggers the bulletin-announcement workflow, which commits a ``Bulletin
\#N Posted'' news item; Netlify deploys again, and the announcement appears
on the home page --- all without further editor action.
...
\texttt{.github/workflows/} & automation: nightly rebuild, bulletin \\
 & \quad announcements (script in \texttt{.github/scripts/}) \\
...
\subsection{Publishing a bulletin}

In the CMS choose \emph{BC Bulletins} $\rightarrow$ \emph{New Bulletin},
fill in the title, issue number (the index page sorts by it), and date,
then click \emph{Choose a file} in the PDF field and upload \emph{from
within that dialog}. Do not use the global \emph{Media} tab for bulletin
PDFs: the Media tab stores files in the site's image-upload folder, while
the bulletin entry's own file picker stores and looks in
\texttt{static/bulletins/}. After \emph{Publish}, the matching news
announcement is created automatically (Section~\ref{sec:stack}); if a
particular bulletin should \emph{not} be announced, simply delete the
auto-created news item afterwards --- it will not come back.
...
\begin{description}
  ...
  \item[A new bulletin's news announcement did not appear.] Check the
  repository's \emph{Actions} tab for the ``Announce new bulletin''
  workflow run triggered by the bulletin's commit; its log explains what it
  did or skipped. The commonest cause is that a news item announcing that
  bulletin number already existed.
